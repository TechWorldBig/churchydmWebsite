...
import AdminMembershipCards from '../components/AdminMembershipCards'
import AdminWeeklyPrograms from '../components/WeeklyPrograms'
import AdminYearlyReport from '../components/AdminYearlyReport'
import AdminAnnualThemeBuilder from '../components/AdminAnnualThemeBuilder'
...
function AdminSectionNav() {
  return <nav aria-label="Admin sections" className="mb-6 flex flex-wrap gap-2"><NavLink end to="/admin" className={({ isActive }) => `secondary-dark-btn ${isActive ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : ''}`}>Management</NavLink><NavLink to="/admin/certificates" className={({ isActive }) => `secondary-dark-btn ${isActive ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : ''}`}>Certificates</NavLink><NavLink to="/admin/yearly-report" className={({ isActive }) => `secondary-dark-btn ${isActive ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : ''}`}>Yearly report</NavLink></nav>
}

export default function Admin({ section = 'management' }: { section?: 'management' | 'certificates' | 'yearly-report' }) {
  ...
  useEffect(() => { if (isAuthed && section === 'management') void loadData() }, [isAuthed, section])
  useEffect(() => {
    if (!isAuthed) return
  ...
  if (section === 'certificates') return <><PageHero eyebrow="Administration" title="Certificates" description="Review eligible members and prepare their recognition certificates." icon={<UserCog size={15} />} /><section className="admin-page py-10 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-8"><div className="flex flex-wrap items-start justify-between gap-3"><AdminSectionNav /><button onClick={() => void handleLogout()} className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-bold text-rose-700 transition hover:bg-rose-100">Log out</button></div><AdminCertificates /><AdminMembershipCards /></div></section></>

  if (section === 'yearly-report') return <><PageHero eyebrow="Administration" title="Yearly ministry report" description="Build the annual ministry theme, then review attendance, programs, events and highlights in English or Tamil." icon={<UserCog size={15} />} /><section className="admin-page py-10 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-8"><div className="flex flex-wrap items-start justify-between gap-3"><AdminSectionNav /><button onClick={() => void handleLogout()} className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-bold text-rose-700 transition hover:bg-rose-100">Log out</button></div><AdminAnnualThemeBuilder /><AdminYearlyReport /></div></section></>

  return <><PageHero eyebrow="Administration" title="Member and attendance management" description="Add complete member profiles, upload photos, and manage attendance records from one place." icon={<UserCog size={15} />} /><section className="admin-page py-10 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-8"><AdminSectionNav /><div className="mb-5 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-500">{loading ? 'Loading shared data...' : 'Connected to shared database'}</p><button onClick={() => void handleLogout()} className="self-end rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-bold text-rose-700 transition hover:bg-rose-100 sm:self-auto">Log out</button></div><div className="grid min-w-0 gap-5 lg:grid-cols-[420px_1fr]">
