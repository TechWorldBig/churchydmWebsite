...
function AdminSectionNav() {
  return <nav aria-label="Admin sections" className="mb-6 flex flex-wrap gap-2"><NavLink end to="/admin" className={({ isActive }) => `secondary-dark-btn ${isActive ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : ''}`}>Management</NavLink><NavLink to="/admin/certificates" className={({ isActive }) => `secondary-dark-btn ${isActive ? 'border-emerald-600 bg-emerald-50 text-emerald-900' : ''}`}>Certificates</NavLink></nav>
}

export default function Admin({ section = 'management' }: { section?: 'management' | 'certificates' }) {
