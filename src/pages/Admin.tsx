...
export default function Admin({ section = 'management' }: { section?: 'management' | 'certificates' | 'yearly-report' }) {
  ...
  if (section === 'yearly-report') return <><PageHero eyebrow="Administration" title="Yearly ministry report" description="Review this year's attendance, programs, events and ministry highlights in English or Tamil." icon={<UserCog size={15} />} /><section className="admin-page py-10 sm:py-16"><div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-8"><div className="flex flex-wrap items-start justify-between gap-3"><AdminSectionNav /><button onClick={() => void handleLogout()} className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-bold text-rose-700 transition hover:bg-rose-100">Log out</button></div><AdminYearlyReport /></div></section></>
