...
import belongTogetherPng from '../assets/belong-together.png'
import missionaryStoryWebp from '../assets/missionary-story.webp'
import musicGif from '../assets/music.gif'
import serveWithPurposePng from '../assets/serve-with-purpose.png'
...
const pillars = [
  ...
  { icon: null, image: belongTogetherPng, title: 'Belong Together', text: 'Create meaningful friendships and a youth community where everyone can participate.' },
  { icon: null, image: serveWithPurposePng, title: 'Serve with Purpose', text: 'Use our gifts in church and community through compassion, outreach and leadership.' },
]
    ...
    <>
      ...
      <section className="home-purpose bg-stone-50 py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-3">{pillars.map((p) => <article key={p.title} className="soft-card"><span className="icon-box icon-box-artwork"><img src={p.image} alt={p.title} className="h-11 w-11 object-contain" /></span><h3 className="mt-6 text-xl font-black">{p.title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{p.text}</p></article>)}</div>
        </div>
      </section>
