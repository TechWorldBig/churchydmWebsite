...
export default function Programs() {
  return <>
    ...
    <section className="py-20"><div className="mx-auto max-w-7xl px-5 lg:px-8">
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{programs.map((program, index) => { const Icon = icons[index]; return <article key={program.id} className="soft-card group relative overflow-hidden"><span className="absolute right-6 top-5 text-5xl font-black text-emerald-700 transition group-hover:text-emerald-800">{program.icon}</span>{program.id === 1 ? <span className="program-bible-icon relative"><img src={holyBibleGif} alt="Holy Bible" /></span> : program.id === 2 ? <span className="program-bible-icon relative"><img src={missionaryStoryWebp} alt="Missionary story" /></span> : program.id === 3 ? <span className="program-bible-icon relative"><img src={musicGif} alt="Music note" /></span> : <span className="icon-box relative"><Icon size={22} /></span>}<p className="mt-7 text-xs font-bold uppercase tracking-[.16em] text-emerald-700">{program.schedule}</p><h2 className="mt-2 text-2xl font-black">{program.title}</h2><p className="mt-3 text-sm leading-7 text-slate-600">{program.description}</p></article> })}</div>
