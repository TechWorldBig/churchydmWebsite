      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <section className="attendance-search-panel mt-12 rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm sm:p-8">
            ...
            {searched && result && <div className="program-result-card mt-7 rounded-3xl bg-slate-50 p-5"><div className="program-result-identity"><div className="member-gif-figure program-result-character"><img src={result.member.gender?.toLowerCase() === 'female' ? memberFemaleGif : memberMaleGif} alt="Member illustration" /></div><div><p className="eyebrow">{result.member.seniority || 'Member'}</p><h3 className="mt-2 text-2xl font-black">{result.member.name}</h3><p className="text-sm text-slate-500">{result.member.role}</p></div></div>{!summaries.length ? <p className="mt-5 rounded-xl bg-white p-4 text-sm text-slate-500">No program points found for the selected dates.</p> : <div className="mt-5 grid gap-4">{summaries.map(summary => <article key={summary.program} className="rounded-2xl bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">Program summary</p><h4 className="mt-1 text-xl font-black">{summary.program}</h4></div><div className="rounded-xl bg-emerald-50 px-3 py-2 text-right"><p className="text-xs font-bold uppercase tracking-[.12em] text-emerald-700">Overall</p><p className="text-lg font-black text-emerald-900">{formatPercentage(summary.overallPercentage)}</p></div></div><div className="mt-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Total points</p><p className="mt-1 font-black">{summary.totalPoints} / {summary.maximumPoints}</p></div><div className="rounded-xl bg-slate-50 p-3"><p className="text-xs text-slate-500">Meetings</p><p className="mt-1 font-black">{summary.rows.length}</p></div><div className="col-span-2 rounded-xl bg-slate-50 p-3 sm:col-span-1"><p className="text-xs text-slate-500">Range</p><p className="mt-1 font-black">{from} – {to || 'Present'}</p></div></div><div className="mt-5"><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-500">Points history</p><div className="mt-2 divide-y divide-slate-100 rounded-xl border border-slate-100">{summary.rows.map(row => <div key={row.id} className="flex items-center justify-between gap-3 px-3 py-3"><div><p className="font-bold">{formatMeetingDate(row.date)}</p><p className="text-xs text-slate-500">Meeting</p></div><div className="program-score-ring h-12 w-12 text-xs" style={scoreStyle(row.questionsAnswered)}><span className="h-9 w-9 text-xs">{row.questionsAnswered}/5</span></div></div>)}</div></div></article>)}</div>}</div>}
          </section>
