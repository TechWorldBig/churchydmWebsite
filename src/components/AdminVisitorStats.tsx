  ...
  return <section className="soft-card mt-6 border border-emerald-100 bg-white shadow-[0_18px_40px_rgba(7,31,25,.08)]">
    ...
    {message ? <p role="alert" className="mt-5 text-sm text-rose-700">{message}</p> : <>
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl bg-emerald-50 p-5 text-emerald-950"><p className="text-xs font-bold uppercase tracking-[.14em] text-emerald-700">Selected day</p><p className="mt-2 text-4xl font-black">{loading ? '-' : stats?.dailyTotal ?? 0}</p><p className="mt-2 text-xs text-emerald-700/70">Unique IPs on {date}</p></div>
      </div>
      ...
      <div className="mt-6"><div className="mb-3"><p className="text-sm font-black text-slate-900">Approximate visitor locations</p><p className="mt-1 text-xs text-slate-500">IP-based geolocation only. City, region, coordinates and time zone may be approximate.</p></div><div className="overflow-hidden rounded-2xl border border-slate-100"><div className="max-h-72 overflow-y-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-[.12em] text-slate-500"><tr><th className="px-4 py-3">Location</th><th className="px-4 py-3">Latitude</th><th className="px-4 py-3">Longitude</th><th className="px-4 py-3">Time zone</th><th className="px-4 py-3">Device</th></tr></thead><tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">Loading locations...</td></tr> : (stats?.locations.length || 0) === 0 ? <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">No IP location data available yet.</td></tr> : stats?.locations.map((location, index) => <tr key={`${location.lastSeenAt}-${index}`}><td className="px-4 py-3 font-bold text-slate-800">{place(location)}</td><td className="px-4 py-3 font-mono text-xs text-slate-500">{location.latitude || '-'}</td><td className="px-4 py-3 font-mono text-xs text-slate-500">{location.longitude || '-'}</td><td className="px-4 py-3 text-slate-500">{location.timezone || '-'}</td><td className="px-4 py-3 text-slate-500">{location.device || 'unknown'}</td></tr>)}</tbody></table></div></div></div>
