...
const finalizeReportLayout = (html: string) => html.replace('</style>', '.signatures{position:static!important;margin-top:48px;break-inside:avoid;page-break-inside:avoid}@media print{@page{margin:0}body{padding:18mm 14mm 20mm}.signatures{position:static!important}}</style>')
const printOrWord = (html: string, format: string, filename: string, setMessage: (message: string) => void) => {
  ...
  const popup = window.open('', '_blank'); if (!popup) { setMessage('Please allow pop-ups to create the PDF.'); return }; popup.document.open(); popup.document.write(html); popup.document.close(); setTimeout(() => { popup.focus(); popup.print() }, 150)
}
