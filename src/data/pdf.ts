export async function downloadHtmlPdf(html: string, filename: string, format: 'a4' | 'card' = 'a4') {
  const { jsPDF } = await import('jspdf')
  const isCard = format === 'card'
  const pdf = new jsPDF({ orientation: isCard ? 'landscape' : 'portrait', unit: 'mm', format: isCard ? [85.6, 54] : 'a4' })
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      pdf.html(frameDocument.body, { x: 0, y: 0, width: isCard ? 85.6 : 190, windowWidth: isCard ? 420 : 1120, autoPaging: 'text', margin: isCard ? 0 : [10, 10, 10, 10], callback: () => resolve(), html2canvas: { scale: isCard ? 2 : 1, useCORS: true } }).catch(reject)
    })
    pdf.save(filename)
