...
const currentYearDate = (value: unknown): boolean => date(value) && (value as string).slice(0, 4) === String(new Date().getFullYear())
  ...
  try {
    ...
    if (req.method !== 'DELETE') {
      if (kind === 'members') {
        ...
        if (req.method === 'POST' && (!text(body.memberId, 100, true) || !text(body.name, 100, true) || !currentYearDate(body.date))) throw new Error()
      } else if (!photo(body.photo) || !body.photo || !currentYearDate(body.date) || (body.description !== undefined && !text(body.description, 2000))) throw new Error()
    }
    return body
  } catch {
