...
export function readMutation(req: any, res: any, kind: 'members' | 'attendance' | 'gallery'): any | null {
  try {
    ...
    if (req.method !== 'DELETE') {
      if (kind === 'members') {
        if (!text(body.name, 100, true) || !['role', 'email', 'phone', 'address', 'focus'].every(key => body[key] === undefined || text(body[key], key === 'focus' ? 2000 : 300)) || (body.gender !== undefined && !['', 'Male', 'Female'].includes(body.gender)) || (body.seniority !== undefined && !['', 'Kutties', 'Junior', 'Senior'].includes(body.seniority)) || (body.dateOfBirth && !date(body.dateOfBirth)) || !photo(body.photo || '')) throw new Error()
