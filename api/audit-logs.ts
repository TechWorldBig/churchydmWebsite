...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    await ensureSchema()
    const rows = await getSql()`SELECT id, actor, action, entity, entity_id AS "entityId", summary, created_at AS "createdAt" FROM audit_logs ORDER BY created_at DESC LIMIT 50`
