...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const rows = await sql`SELECT id, member_id AS "memberId", name, program, seniority, date, questions_answered AS "questionsAnswered" FROM program_points WHERE LOWER(name) = LOWER(${query.name}) AND (${query.program || ''} = '' OR program = ${query.program || ''}) AND (${query.from || ''} = '' OR date >= ${query.from || ''}) AND (${query.to || ''} = '' OR date <= ${query.to || ''}) ORDER BY date DESC, created_at DESC`
